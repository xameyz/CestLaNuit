...
class MyComponent extends JComponent implements MouseListener {
	...
	public void mouseClicked(MouseEvent clic) {
		...
		for (int i = 0; i < this.waypoints.size(); i++) {
			...
			//
			if (waypoint_courant_Point.x - mousePosition_Point.x < (20 - this.current_zoom)
					&& mousePosition_Point.y - waypoint_courant_Point.y < (20 - this.current_zoom)
					&& mousePosition_Point.x - waypoint_courant_Point.x < (20 - this.current_zoom)) {
